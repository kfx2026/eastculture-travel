...
function _renderCityDetail(city, app) {
  ...
  // Cuisine
  var cuHtml = '';
  var cuisineList = city.cuisine || city.food || [];
  for (var m = 0; m < cuisineList.length; m++) {
    var cu = cuisineList[m];
    cuHtml += '' +
      '<div class="cuisine-card">' +
        ...
        '<div class="cu-body">' +
          ...
          '<div class="cu-meta">' +
            (cu.where ? '<span>📍 ' + tl(cu.where) + '</span>' : '') +
            (cu.price ? '<span>💰 ' + tl(cu.price) + '</span>' : '') +
          '</div>' +
          (cu.must_try ? '<div class="cu-try">💡 ' + tl(cu.must_try) + '</div>' : '') +
        '</div>' +
      '</div>';
  }
  ...
  // Culture
  var clHtml = '';
  var cultureList = city.culture || [];
  for (var n = 0; n < cultureList.length; n++) {
    var cl = cultureList[n];
    clHtml += '' +
